...
public class ServerRpcProvider {
	 ...
	 * {@link WaveWebSocketServlet} as init param. It defines the response
	 * buffer size.
	 */
	 ...
	 */
	public ServerRpcProvider(InetSocketAddress[] httpAddresses, String[] resourceBases, Executor threadPool,
			SessionManager sessionManager, org.eclipse.jetty.server.SessionManager jettySessionManager,
			String sessionStoreDir, boolean sslEnabled, String sslKeystorePath, String sslKeystorePassword) {
	 ...
	 */
	public ServerRpcProvider(InetSocketAddress[] httpAddresses, String[] resourceBases, SessionManager sessionManager,
			org.eclipse.jetty.server.SessionManager jettySessionManager, String sessionStoreDir, boolean sslEnabled,
			String sslKeystorePath, String sslKeystorePassword, Executor executor) {
		this(httpAddresses, resourceBases, executor, sessionManager, jettySessionManager, sessionStoreDir, sslEnabled,
	...
	public ServerRpcProvider(@Named(CoreSettings.HTTP_FRONTEND_ADDRESSES) List<String> httpAddresses,
		...
		this(parseAddressList(httpAddresses, websocketAddress), resourceBases.toArray(new String[0]), sessionManager,
				jettySessionManager, sessionStoreDir, sslEnabled, sslKeystorePath, sslKeystorePassword,
				executorService);
	}
	...
	private void restoreSessions() {
		try {
			...
			hashSessionManager
					.setStoreDirectory(FileUtils.createDirIfNotExists(sessionStoreDir, "Session persistence"));
	...
	private List<Connector> getSelectChannelConnectors(InetSocketAddress[] httpAddresses) {
		...
		if (sslEnabled) {
			...
			// Note: we only actually needed client auth for
	 ...
	 */
	@Singleton
	@AtmosphereHandlerService(path = "/atmosphere/wave", interceptors = {
			AtmosphereClientInterceptor.class }, broadcasterCache = UUIDBroadcasterCache.class)
	public static class WaveAtmosphereService implements AtmosphereHandler {
		...
		public void onRequest(AtmosphereResource resource) throws IOException {
			...
			AtmosphereResourceSession resourceSession = resource.getAtmosphereConfig().sessionFactory()
					.getSession(resource);

			AtmosphereChannel resourceChannel = resourceSession.getAttribute(WAVE_CHANNEL_ATTRIBUTE,
					AtmosphereChannel.class);

			if (resourceChannel == null) {

				ParticipantId loggedInUser = provider.sessionManager
						.getLoggedInUser(resource.getRequest().getSession(false));
			...
			resource.setBroadcaster(resourceChannel.getBroadcaster()); // on
																		// every
																		// request
			...
			if (resource.getRequest().getMethod().equalsIgnoreCase("POST")) {
				...
				if (body != null && String.class.isAssignableFrom(body.getClass())) {
					resourceChannel.onMessage((String) body);
				}
			}
		}
