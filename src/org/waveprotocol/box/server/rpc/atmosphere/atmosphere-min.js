...
}}}(this,function(){var c="2.3.1-javascript",a={},e,d=false,h=[],g=[],f=0,b=Object.prototype.hasOwnProperty;
...
}function E(){if(q.enableProtocol&&!q.disableDisconnect&&!q.firstMessage){var aL="X-Atmosphere-Transport=close&X-Atmosphere-tracking-id="+q.uuid;
...
k(q);
if(!q.enableXDR&&window.location.host&&aN.origin&&aN.origin!==window.location.protocol+"//"+window.location.host){a.util.log(q.logLevel,["Origin was not "+window.location.protocol+"//"+window.location.host]);
return
...
}if(at.closedByClientTimeout||(q.handleOnlineOffline&&d)){if(q.reconnectId){clearTimeout(q.reconnectId);
delete q.reconnectId
}return
}af(aK);
at.state="closed";
...
at.messages=[];
ag(true)
}}else{a.util.log(q.logLevel,["Websocket reconnect maximum try reached "+az]);
...
}if(aM.reconnect||(aM.suspend&&v)){var aJ=0;
...
}if(aK>0){q.reconnectId=setTimeout(function(){M(aM)
},aK)
}else{M(aM)
...
l<j.length;
l++){var k=j[l];
if(k.request.handleOnlineOffline){k.close();
clearTimeout(k.response.request.id);
if(k.heartbeatTimer){clearTimeout(k.heartbeatTimer)
}}}}});
...
j<h.length;
j++){if(h[j].request.handleOnlineOffline){h[j].init();
h[j].execute()
}}}d=false
});
return a
