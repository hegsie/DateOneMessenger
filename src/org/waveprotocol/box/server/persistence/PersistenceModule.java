...
import org.waveprotocol.box.server.persistence.mongodb.MongoDbAccountStore;
import org.waveprotocol.box.server.persistence.mongodb.MongoDbAttachmentStore;
...
import org.waveprotocol.box.server.persistence.mongodb.MongoDbSignerInfoStore;
...
public class PersistenceModule extends AbstractModule {
  ...
  private void bindCertPathStore() {
    if (signerInfoStoreType.equalsIgnoreCase("memory")) {
      ...
      bind(DB.class).toInstance(getMongoDbProvider().getDB());
      bind(CertPathStore.class).to(MongoDbSignerInfoStore.class).in(Singleton.class);
    } else {
  ...
  private void bindAttachmentStore() {
    if (attachmentStoreType.equalsIgnoreCase("disk")) {
      ...
      bind(DB.class).toInstance(getMongoDbProvider().getDB());
      bind(AttachmentStore.class).to(MongoDbAttachmentStore.class).in(Singleton.class);
    } else {
  ...
  private void bindAccountStore() {
    if (accountStoreType.equalsIgnoreCase("memory")) {
      ...
      bind(DB.class).toInstance(getMongoDbProvider().getDB());
      bind(AccountStore.class).to(MongoDbAccountStore.class).in(Singleton.class);
    } else {
  ...
  private void bindDeltaStore() {
    if (deltaStoreType.equalsIgnoreCase("memory")) {
      ...
      bind(DB.class).toInstance(getMongoDbProvider().getDB());
      bind(DeltaStore.class).to(MongoDbDeltaStore.class).in(Singleton.class);
    } else {
