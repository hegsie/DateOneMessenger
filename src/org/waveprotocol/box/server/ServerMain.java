...
import com.greycells.dateone.server.DateOneServiceImpl;
import com.greycells.dateone.server.FileUploadServlet;
import com.greycells.dateone.server.ImageServlet;
...
import org.waveprotocol.box.server.rpc.DateOneClientServlet;
...
private static Module buildFederationModule(Injector settingsInjector, boolean enableFederation)
  ...
  private static void initializeServlets(Injector injector, ServerRpcProvider server) {
    ...
    server.addServlet("/images", ImageServlet.class);
    
    ServletHolder upload = server.addServlet("*.gupld", FileUploadServlet.class);
    upload.setInitParameter("slowUploads", "false");
    upload.setInitParameter("maxSize", "3145728");

    ServletHolder dateOne = server.addServlet("/dateone/dateOneService", DateOneServiceImpl.class);
    dateOne.setInitOrder(-1);
    
    String gadgetHostName = injector
    ...
    server.addServlet("/", DateOneClientServlet.class);
